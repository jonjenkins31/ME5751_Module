...
class path_planner:
	...
	def plan_path(self):
		...
		p_raw=[]
		p_raw.append(1)													# List of nodes  formed
		...
		while  path_found == False:								# loop as long as a list of neighbors to solve for exists
			...
			p_normalized= normalize(p_raw)									# List of nodes  formed
			random_node_c = random.choices(node_list,weights=p_normalized,k=1)  			# Randomly select a node c from the list of nodes
			#random_node_c = random.choice(node_list)  			# Randomly select a node c from the list of nodes
			random_node_c = random_node_c[0]   
			...
			if  random_node_c_prime_valid == True:								# Perferm only if new node is valid
				...
				if(hit_obstacle==False):										# Path is clear
					...
					parent_index=node_list.index(random_node_c) 
					parent_cost=p_raw[parent_index]
					bias= (manhattan(start,random_node_c_prime)/manhattan(goal,random_node_c_prime))
				...
				#	new_cost = cost_so_far[current] + search_map[next] 															# Cell Cost = Actual Cost + Search Map Grid Cost 
					p_raw.append(bias)													# List of nodes  formed
...
def normalize(input):
	norm=[]
	for i in input:															# check if straightline from the two nodes hits any obstacle
		norm.append(float(i)/sum(input))									# generate the manhattan distance value from goal to current position
	return norm
